#!/usr/bin/env python3
"""
Event Consumer - Processes events from Redis Stream
=====================================================
            ...
            for stream_name, stream_messages in messages:
                for msg_id, msg_data in stream_messages:
                    try:
                        ...
                        logger.info(f"[{msg_id.decode()}] Processing match {match_id}")
                        ...
                        logger.info(f"✓ ACK message {msg_id.decode()}")
                        
                    except Exception as e:
                        logger.error(f"Failed to process message {msg_id.decode()}: {e}", exc_info=True)
...
def main():
    logger.info("=" * 60)
    logger.info("Starting VAEP Consumer")
    logger.info("=" * 60)
