...
Usage:
    python shotmap_generator_redis.py
...
class ShotmapGeneratorRedis:
    ...
    def load_match_data(self):
        ...
        try:
            ...
            # Convert numeric columns first before filtering
    ...
    def generate_shotmap_data(self):
        ...
        try:
            ...
            # Debug: Print available columns in shotmap data
            print(f"🔍 Available columns in shotmap data: {list(self.shotmap_data.columns)}")
    ...
    def create_frontend_output(self):
        ...
        # Create simplified structure (matching original shotmap format exactly)
        output = {
        ...
        # Process each shot and deduplicate (using available columns from df_match_shotmap)
        seen_shots = set()
        
        for _, shot in self.shotmap_data.iterrows():
            # Create a unique key for deduplication
            shot_key = (
                int(shot["team_id"]) if pd.notna(shot["team_id"]) else None,
                float(shot["x_both"]) if pd.notna(shot["x_both"]) else None,
                float(shot["y_both"]) if pd.notna(shot["y_both"]) else None,
                float(shot["xg"]) if pd.notna(shot["xg"]) else 0.0,
                int(shot["goal"]) if pd.notna(shot["goal"]) else 0
            )
            
            # Skip if we've already seen this exact shot
...
if __name__ == "__main__":
    ...
    for match_id in match_ids:
        ...
        generator = ShotmapGeneratorRedis(match_id)
